...
// Smoothing: EMA toward each new detection to kill BlazeFace's frame-to-frame flicker. The
// centroid-reflection in reduce() roughly doubles landmark jitter, so this runs heavier than usual.
const SMOOTH = 0.18;     // higher = snappier/noisier, lower = calmer/laggier
...
// BlazeFace's two eye keypoints have a small, consistent vertical offset (measured: a level face
// still reports the left eye ~0.02 lower than the right), which the portrait aspect amplifies into a
// visible ~10° lean — every filter looks crooked. ROLL_FIX rotates the whole constellation about the
// pivot (in screen-proportional space) to null it. It's a CONSTANT, so real head tilts still track on
// top of it. Degrees; tune against the Debug lens until the two eye dots are level. Flip the sign if
...
export function useFaceTracking(mirror = true) {
  ...
  const setAspect = useCallback((fa: number) => {
    if (fa > 0 && Math.abs(fa - frameAspectRef.current) > 0.001) {
      ...
      setFrameAspect(fa);
      console.log('[faceTracking] frameAspect (measured) =', fa);
    }
  }, []);
  ...
  const frameProcessor = useFrameProcessor((frame) => {
    ...
    runAtTargetFps(LIVE_FPS, () => {
      ...
      if (res && res.points) { pushJs(reduce(res.points, aspect), 'ok'); }
      else { pushJs(null, (res && res.err) || 'null'); }
    });
  }, [pushJs, setAspectJs, mirror]);
