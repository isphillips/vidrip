...
// here, and this page bounces back into the app via the vidrip:// deep link.
Deno.serve((req) => {
  ...
  const deepLink = error
    ? `vidrip://oauth?error=${encodeURIComponent(error)}` +
      `&error_description=${encodeURIComponent(errorDescription)}` +
      `&state=${encodeURIComponent(state)}`
    : `vidrip://oauth?code=${encodeURIComponent(code)}&state=${encodeURIComponent(state)}`;
